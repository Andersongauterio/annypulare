...
import br.com.annypularebackend.services.UnidadeMedidaService;
...
public class UnidadeMedidaResource {
	...
	@GetMapping
	public ResponseEntity<Page<UnidadeMedidaDTO>> findAll(Pageable pageable) {

		// PARAMETROS: page, size, sort

		Page<UnidadeMedidaDTO> list = service.findAllPaged(pageable);
		return ResponseEntity.ok().body(list);
	}

	@GetMapping(value = "/{id}")
	public ResponseEntity<UnidadeMedidaDTO> findById(@PathVariable Long id) {
		UnidadeMedidaDTO dto = service.findById(id);
		return ResponseEntity.ok().body(dto);
	}

	@PostMapping
	public ResponseEntity<UnidadeMedidaDTO> insert(@RequestBody UnidadeMedidaDTO dto) {
